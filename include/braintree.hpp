...
// The StatefulSelector composite ticks each child node in order, and remembers
// what child it prevously tried to tick. If a child succeeds or runs, the
// stateful selector returns the same status. In the next tick, it will try to
// run the next child or start from the beginning again. If all children fails,
// only then does the stateful selector fail.
...
// The StatefulSequence composite ticks each child node in order, and remembers
// what child it prevously tried to tick. If a child fails or runs, the stateful
// sequence returns the same status. In the next tick, it will try to run the
// next child or start from the beginning again. If all children succeeds, only
// then does the stateful sequence succeed.
...
// The Succeeder decorator returns success, regardless of what happens to the
// child.
...
// The Failer decorator returns failure, regardless of what happens to the
// child.
...
// The Inverter decorator inverts the child node's status, i.e. failure becomes
// success and success becomes failure. If the child runs, the Inverter returns
// the status that it is running too.
...
// The Repeater decorator repeats infinitely or to a limit until the child
// returns success.
...
// The UntilSuccess decorator repeats until the child returns success and then
// returns success.
...
// The UntilFailure decorator repeats until the child returns fail and then
// returns success.

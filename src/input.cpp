...
static void key_callback(
    ...
    int mods) {
  auto inputManager =
      glfw::get_user_pointer<manager>(window);
  inputManager->enqueue(
      event<key>{{keycode, action}, vka::Clock::now()});
}
...
static void mouse_button_callback(
  ...
  auto inputManager =
      glfw::get_user_pointer<manager>(window);
  inputManager->enqueue(
      event<mouse>{{button, action}, vka::Clock::now()});
}

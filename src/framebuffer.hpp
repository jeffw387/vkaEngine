...
struct framebuffer_builder {
  tl::expected<std::unique_ptr<framebuffer>, VkResult>
  build(VkDevice device) {
    ...
    auto result = vkCreateFramebuffer(
        device, &createInfo, nullptr, &framebufferHandle);
    if (result != VK_SUCCESS) {
      return tl::make_unexpected(result);
    }

    return std::make_unique<framebuffer>(
        device, framebufferHandle);
  }

  framebuffer_builder& render_pass(
      VkRenderPass renderPass) {
    m_renderPass = renderPass;
    return *this;
  }

  framebuffer_builder& dimensions(
      uint32_t width = 1,
      uint32_t height = 1,
      uint32_t layers = 1) {
    m_width = width;
  ...
  framebuffer_builder& attachments(

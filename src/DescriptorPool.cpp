...
DescriptorSet::DescriptorSet(VkDescriptorSet set, DescriptorBindings descriptors)
    : set(set), bindings(descriptors) {
...
std::shared_ptr<DescriptorSet> DescriptorPool::allocateDescriptorSet(
  ...
  return std::make_shared<DescriptorSet>(vkSet, std::move(bindings));
}

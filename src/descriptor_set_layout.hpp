...
struct descriptor_set_layout {
  ...
  const std::vector<VkDescriptorSetLayoutBinding>& layout_bindings() const noexcept {
    return m_bindings;
  }
private:
  VkDevice m_device = {};
...
struct descriptor_set_layout_builder {
  tl::expected<std::unique_ptr<descriptor_set_layout>, VkResult>
  build(VkDevice device) {
    ...
    return std::make_unique<descriptor_set_layout>(
      device,
      layout,
      m_bindings);
  }
  ...
  descriptor_set_layout_builder& image(
      uint32_t binding, 
      uint32_t count, 
      VkShaderStageFlags stageFlags) {
    VkDescriptorSetLayoutBinding bindingDescription = {};

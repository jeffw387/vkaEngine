...
std::shared_ptr<spdlog::logger> multi_logger::get() {
  if (!multi_logger::multilogger) {
    ...
    multi_logger::stdoutSink = std::make_shared<
        spdlog::sinks::stdout_color_sink_mt>();
    multi_logger::stdoutSink->set_level(
        spdlog::level::info);
    multi_logger::stderrSink = std::make_shared<
        spdlog::sinks::stderr_color_sink_mt>();
    multi_logger::stderrSink->set_level(
        spdlog::level::info);
    std::vector<spdlog::sink_ptr> sinks = {
        multi_logger::fileSink, multi_logger::stderrSink};

...
using border_color = std::variant<black_border, white_border, transparent_border>;
...
struct sampler_builder {
  tl::expected<std::unique_ptr<sampler>, VkResult> build(VkDevice device) {
    ...
    createInfo.borderColor = std::visit(border_visitor, m_borderColor, m_borderType);
  ...
  sampler_builder& address_modes(VkSamplerAddressMode u, VkSamplerAddressMode v, VkSamplerAddressMode w) {
    m_uAddressMode = u;
    m_vAddressMode = v;
    m_wAddressMode = w;
    return *this;
